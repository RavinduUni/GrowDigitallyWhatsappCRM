...
const ChatWindow = ({
  ...
  const handleToggleAI = async () => {
    ...
    setToggling(true);
    try { await onToggleAI(); } finally { setToggling(false); }
  };

  const handleTakeover = async () => {
    if (!onTakeover) return;
    setTakingOver(true);
    try { await onTakeover(); } finally { setTakingOver(false); }
  };
    ...
    return (
      <main className="flex-1 flex flex-col items-center justify-center bg-[#F0F2F5] min-w-0">
        <div className="flex flex-col items-center gap-md text-center">
          ...
          <h2 className="font-h1 text-h1 text-on-surface">WhatsApp CRM Monitor</h2>
          <p className="font-body-md text-body-md text-on-surface-variant">
            Select a conversation from the list to start viewing messages and replying to customers.
          </p>
        </div>
      </main>
    );
  ...
  return (
    <main className="flex-1 flex flex-col bg-[#F0F2F5] min-w-0 relative z-0">
      ...
      <header className="h-[72px] bg-surface border-b border-panel-border flex items-center justify-between px-md shrink-0 shadow-sm z-10">
        ...
        >
          <div className="w-10 h-10 rounded-full bg-secondary-container text-on-secondary-container flex items-center justify-center font-h2 text-h2 uppercase flex-shrink-0">
        ...
        <div className="flex items-center space-x-sm shrink-0">
          ...
          <div className="flex items-center bg-surface-container border border-outline-variant rounded-full p-1">
            <button
              ...
              className={`rounded-full px-3 py-1 font-label-sm text-label-sm flex items-center transition-all cursor-pointer ${
                aiEnabled ? 'bg-surface text-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'
              }`}
              id="toggle-ai-btn"
            >
          ...
          <button
            ...
            id="takeover-btn"
          >
            <span className="material-symbols-outlined text-[18px] mr-1">pan_tool_alt</span>
          ...
          <div className="relative" ref={menuRef}>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-full mt-1 w-52 bg-surface-container-lowest border border-panel-border rounded-xl shadow-overlay z-50 py-1 overflow-hidden">
                {/* Toggle panel item */}
                <button
                  onClick={() => { onTogglePanel(); setMenuOpen(false); }}
